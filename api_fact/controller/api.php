        ...
        if ($documento->enviado_sunat == '1'):
            ...
            if($documento->comision_delivery > 0){
                $dely_pr = array("codigo_producto" => "DELIV001",
                    ...
                    "cantidad" => "1.00" ,
                    "valor_unitario" => ($deco[0]['codigo_afectacion'] == 10)? number_format(($documento->comision_delivery/(1+ igv_dec)), 6, '.', '') : number_format(($documento->comision_delivery), 6, '.', ''),
                    "precio_unitario" => number_format($documento->comision_delivery, 2, '.', '') ,
                    "valor_venta" => ($deco[0]['codigo_afectacion'] == 10)? number_format(($documento->comision_delivery/(1+ igv_dec)), 2, '.', '') : number_format(($documento->comision_delivery), 2, '.', ''),
                    "total_igv" => ($deco[0]['codigo_afectacion'] == 10)? number_format($documento->comision_delivery-($documento->comision_delivery/(1 + igv_dec)), 2, '.', '') : "0");
            ...
            foreach ($resultado_detalle as $detalle ){
                $item = $item + 1;
                $codafectacion = $detalle['codigo_afectacion'];
            ...
            if($documento->consumo == '1'){
                $productos[] = [
                    ...
                    "txtCANTIDAD_DET" => "1",
                    "txtPRECIO_DET" => number_format((($documento->total +$documento->descuento_monto + $documento->comision_delivery)), 6, '.', ''), #TOTAL DEL PRODUCTO
                    "txtPRECIO_TIPO_CODIGO" => "01", #Dejarlo como esta
                    // "txtIGV" => number_format(($documento->total - ($documento->total / ((1+ $documento_igv)))), 2, '.', ''), // total - base
                    "txtIGV" => ($codafectacion == '20')? '0.00' : number_format(((($documento->total +$documento->descuento_monto + $documento->comision_delivery) / ((1+ igv_dec)))* igv_dec), 2, '.', ''), 
                    "txtISC" => "0",  #Dejarlo como esta
                    "txtIMPORTE_DET" => ($codafectacion == '20')?  number_format((($documento->total+$documento->descuento_monto + $documento->comision_delivery)), 2, '.', '') : number_format((($documento->total+$documento->descuento_monto + $documento->comision_delivery) / ((1+ igv_dec))), 2, '.', ''),
                    "txtCOD_TIPO_OPERACION" => $codafectacion, #Dejarlo como esta
                    "txtCODIGO_DET" => "CON0001",
                    "txtDESCRIPCION_DET" => $documento->consumo_desc,
                    "txtPRECIO_SIN_IGV_DET" => ($codafectacion == '20')?  number_format((($documento->total+$documento->descuento_monto + $documento->comision_delivery)), 6, '.', '') : number_format((($documento->total+$documento->descuento_monto + $documento->comision_delivery) / ((1+ igv_dec))), 6, '.', '') // precio /igv
                ];
            }  
            ...
            //CALCULOS INICIO
                if($total_ope_gravadas > 0){
                   ...
                   $total_gravadas = (($documento->total + $documento->comision_delivery) - $documento->descuento_monto) / igv_dec2;

                    $total_exoneradas = "0";

                    $sub_tot = number_format(((($documento->total + $documento->comision_delivery) - $documento->descuento_monto) - ((($documento->total + $documento->comision_delivery) - $documento->descuento_monto) / igv_dec2)), 2, '.', '');

                    $sub_tot1 = number_format(((($documento->total + $documento->comision_delivery) - $documento->descuento_monto) / igv_dec2), 2, '.', '');
                    $totales = number_format((($documento->total + $documento->comision_delivery) - $documento->descuento_monto), 2, '.', '');
                    $totales_2 = $totales;
                    $dscto_global = number_format(($documento->descuento_monto / igv_dec2), 2, '.', '');
                    $total_descuento = "0";
                } else {
            ...
            $data = [
                ...
                "porcentaje_igv" => igv_int, #El IGV debe ser 18, si lo tienes en igv_dec, conviértelo a 18
                "total_igv" =>  $sub_tot,
                "total_isc" => "0",
                "total_otr_imp" => "0",
                "total" => $totales,
