...
class Compra_Model extends Model
{
    ...
    public function compra_crud_create($data)
    {
        ...
        {   
            ...
            $sql = "INSERT INTO tm_compra (id_prov, id_tipo_compra, id_tipo_doc, id_usu, fecha_c, hora_c, serie_doc, num_doc, igv, total, descuento, adicional, fecha_reg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);";
            $this->db->prepare($sql)
                ->execute(
                    array(
                        $data['id_prov'],
            ...
            foreach($data['items'] as $d)
            {
                ...
                // Actualización en tm_producto_pres
                $sql_update = "UPDATE tm_producto_pres SET precio_compra = ? WHERE id_pres = ?";
                $this->db->prepare($sql_update)->execute(array($d['precio_insumo'], $d['id_ins_insumo']));

                // Actualización en tm_insumo 
                $sql_update2 = "UPDATE tm_insumo SET cos_uni = ? WHERE id_ins = ?";
                $this->db->prepare($sql_update2)->execute(array($d['precio_insumo'], $d['id_ins_insumo']));
            }
