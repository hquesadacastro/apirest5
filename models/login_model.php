...
class Login_Model extends Model
{
	...
	public function run()
	{
		...
		if ($count_u > 0) {
			...
			Session::set('cod_admin', $data_s['cod_admin']); //funcion codigo de seguridad adicional
			Session::set('opc_01', $data_s['opc_01']);
			Session::set('opc_02', $data_s['opc_02']);
			Session::set('opc_03', $data_s['opc_03']);
			Session::set('print_com_combo', $data_s['print_com_combo']);

			$dup = $this->db->prepare("SELECT id_permiso FROM tm_permisos_asignados WHERE id_usuario = :id_usuario");
			$dup->execute(array(
				':id_usuario' => $data_u['id_usu']
			));
	
			$data_up = $dup->fetchAll();
			
			Session::set('data_up', $data_up);
