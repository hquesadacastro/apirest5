	...
	public function tablero_datos()
    {
        try
        {
            ...
            */
            ...
            $sql_ventas = $this->db->prepare("SELECT IFNULL(SUM(pago_efe),0) AS pago_efe, IFNULL(SUM(pago_tar),0) AS pago_tar, IFNULL(SUM(pago_bil),0) AS pago_bil, IFNULL(SUM(pago_tra),0) AS pago_tra, IFNULL(SUM(pago_lin),0) AS pago_lin, IFNULL(SUM(desc_monto),0) AS descuento, IFNULL(SUM(comis_tar),0) AS comis_tar, IFNULL(SUM(comis_del),0) AS comis_delivery, IFNULL(SUM(total),0) AS total FROM v_ventas_con WHERE /*(fec_ven >= ? AND fec_ven < ?) AND*/ id_apc = ? AND estado <> 'i'");
            ...
            $sql_canal_salon = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total_ventas FROM tm_venta WHERE id_tipo_pedido = 1 AND estado = 'a' AND id_apc = ?");
            $sql_canal_salon->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_salon = $sql_canal_salon->fetch(PDO::FETCH_OBJ);

            $sql_canal_mostrador = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total_ventas FROM tm_venta WHERE id_tipo_pedido = 2 AND estado = 'a' AND id_apc = ?");
            $sql_canal_mostrador->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_mostrador = $sql_canal_mostrador->fetch(PDO::FETCH_OBJ);

            $sql_canal_delivery = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total_ventas FROM tm_venta WHERE id_tipo_pedido = 3 AND estado = 'a' AND id_apc = ?");
            $sql_canal_delivery->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_delivery = $sql_canal_delivery->fetch(PDO::FETCH_OBJ);

            $sql_canal_salon_i = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total_ventas FROM tm_venta WHERE id_tipo_pedido = 1 AND estado = 'i' AND id_apc = ?");
            $sql_canal_salon_i->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_salon_i = $sql_canal_salon_i->fetch(PDO::FETCH_OBJ);

            $sql_canal_mostrador_i = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total_ventas FROM tm_venta WHERE id_tipo_pedido = 2 AND estado = 'i' AND id_apc = ?");
            $sql_canal_mostrador_i->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_mostrador_i = $sql_canal_mostrador_i->fetch(PDO::FETCH_OBJ);

            $sql_canal_delivery_i = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total_ventas FROM tm_venta WHERE id_tipo_pedido = 3 AND estado = 'i' AND id_apc = ?");
            ...
            */
            ...
            $productos_agotarse = array();
            $insumos_agotarse = array();

            $stm = $this->db->prepare("SELECT tpp.id_pres, tp.nombre, tpp.presentacion, tpp.stock_min FROM tm_producto_pres AS tpp INNER JOIN tm_producto AS tp ON tp.id_prod=tpp.id_prod WHERE tpp.stock_min > 0");
            $stm->execute();
            $sm = $stm->fetchAll(PDO::FETCH_OBJ);
            foreach($sm as $k => $d) {
                $stock_minimo = $d->stock_min;

                $datos_ingresos = $this->db->query("SELECT IFNULL(SUM(cant),0) AS ingresos FROM tm_inventario WHERE (id_tipo_ope='1' OR id_tipo_ope='3') and id_tipo_ins='2' and id_ins='".$d->id_pres."'")
                ->fetch(PDO::FETCH_OBJ);
                $datos_egresos = $this->db->query("SELECT IFNULL(SUM(cant),0) AS egresos FROM tm_inventario WHERE (id_tipo_ope='2' OR id_tipo_ope='4')  and id_tipo_ins='2' and id_ins='".$d->id_pres."'")
                ->fetch(PDO::FETCH_OBJ); 

                $datos_ingresos = json_decode(json_encode($datos_ingresos),true);
                $datos_egresos = json_decode(json_encode($datos_egresos),true);

                $ingresos = $datos_ingresos['ingresos'];
                $egresos = $datos_egresos['egresos'];

                $stock_actual = $ingresos - $egresos;

                if ($stock_actual <= $stock_minimo) {
                    if ($stock_actual == $stock_minimo) {
                        $sm[$k]->{'estado_actual'} = "<button class='producto_agotado'>Agotado</button>";
                    } else {
                        $sm[$k]->{'estado_actual'} = "<button class='producto_por_agotarse'>Pocas Unidades</button>";
                    }
                    $sm[$k]->{'stock_actual'} = $stock_actual;
                    $sm[$k]->{'url'} = URL;
                    array_push($productos_agotarse, $d);
                }
            }

            $stm = $this->db->prepare("SELECT ti.id_ins, ti.nomb_ins, ti.stock_min, tic.descripcion FROM tm_insumo AS ti INNER JOIN tm_insumo_catg AS tic ON tic.id_catg=ti.id_catg WHERE ti.stock_min > 0");
            $stm->execute();
            $sm = $stm->fetchAll(PDO::FETCH_OBJ);
            foreach($sm as $k => $d) {
                $stock_minimo = $d->stock_min;

                $datos_ingresos = $this->db->query("SELECT IFNULL(SUM(cant),0) AS ingresos FROM tm_inventario WHERE (id_tipo_ope='1' OR id_tipo_ope='3') and id_tipo_ins='1' and id_ins='".$d->id_ins."'")
                ->fetch(PDO::FETCH_OBJ);
                $datos_egresos = $this->db->query("SELECT IFNULL(SUM(cant),0) AS egresos FROM tm_inventario WHERE (id_tipo_ope='2' OR id_tipo_ope='4')  and id_tipo_ins='1' and id_ins='".$d->id_ins."'")
                ->fetch(PDO::FETCH_OBJ); 
                ...
                if ($stock_actual <= $stock_minimo) {
                    ...
                    $sm[$k]->{'stock_actual'} = $stock_actual;
                    $sm[$k]->{'url'} = URL;
                    array_push($insumos_agotarse, $d);
                }
            }            

            $stock_minimo_datos = array("Productos" => $productos_agotarse, "Insumos" => $insumos_agotarse);            

            $data = array("Ventas" => $ventas,"Egresos" => $egresos,"Ingresos" => $ingresos,"Platos" => $platos,"Productos" => $productos, "Pollostock" => $pollos_stock, "Pollosvendidos" => $pollos_vendidos, "CanalSalon" => $canal_salon, "CanalMostrador" => $canal_mostrador, "CanalDelivery" => $canal_delivery, "CanalSalonAnulados" => $canal_salon_i, "CanalMostradorAnulados" => $canal_mostrador_i,"CanalDeliveryAnulados" => $canal_delivery_i,"StockMinimo" => $stock_minimo_datos/*"data3" => $m,"data4" => $tp,"data5" => $me,"data6" => $vm,"data7" => $vmo,"data8" => $mo,*//*,"data11" => $ma,"data12" => $moa*/);
