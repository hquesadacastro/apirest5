...
class Venta_Model extends Model
{
    ...
    public function TipoPago()
    {
        try
        {   
            return $this->db->selectAll('SELECT * FROM tm_tipo_pago WHERE estado = "a" ORDER BY id_pago ASC, descripcion ASC');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    */
    ...
    public function listarPedidos($data)
    {
        ...
        {   
            if($data['codpagina'] == '1'){
                $stm = $this->db->prepare("SELECT dp.id_pedido, dp.id_pres, SUM(dp.cantidad) AS cantidad, dp.precio, dp.comentario, dp.estado, p.nombre_mozo FROM tm_detalle_pedido AS dp INNER JOIN v_pedido_mesa AS p ON p.id_pedido = dp.id_pedido WHERE dp.id_pedido = ? AND dp.estado <> 'z' AND dp.cantidad > 0 GROUP BY dp.id_pres, dp.precio ORDER BY dp.secuencia DESC");
            } else {
                $stm = $this->db->prepare("SELECT dp.id_pedido, dp.id_pres, SUM(dp.cantidad) AS cantidad, dp.precio, dp.comentario, dp.estado FROM tm_detalle_pedido AS dp WHERE dp.id_pedido = ? AND dp.estado <> 'z' AND dp.cantidad > 0 GROUP BY dp.id_pres, dp.precio ORDER BY dp.secuencia DESC");
            }
    ...
    public function listarPedidosDetalle($data)
    {
        ...
        {   
            ...
            /* Traemos el detalle */
            $c->{'Detalle'} = $this->db->query("SELECT id_pedido,id_pres,SUM(cant) AS cant, precio, comentario, estado FROM tm_detalle_pedido WHERE id_pedido = ".$c->id_pedido." AND estado <> 'z' AND cant > 0 GROUP BY id_pres, precio ORDER BY secuencia DESC")
        ...
        {   
            if ($data['codtipoped'] == 3){
                if($data['codrepartidor'] == 1){
                    $campo = ',pro_cos';
                    $variable = ' ORDER BY pro_pre ASC';
                } else{
                    $campo = ',pro_cos_del AS pro_cos';
                    $variable = ' AND pro_cos_del > 0 ORDER BY pro_pre ASC';
                }
            } else {
                $campo = ',pro_cos';
                $variable = ' ORDER BY pro_pre ASC';
    ...
    public function listarProdsMasVend($data)
    {
        ...
        {   
            if ($data['codtipoped'] == 3){
                if($data['codrepartidor'] == 1){
                    $campo = ',p.pro_cos';
                    $variable = 'GROUP BY dv.id_prod ORDER BY p.pro_pre ASC';
                } else{
                    $campo = ',p.pro_cos_del AS pro_cos';
                    $variable = ' AND p.pro_cos_del > 0 GROUP BY dv.id_prod ORDER BY p.pro_pre ASC';
                }
            } else {
                $campo = ',p.pro_cos';
                $variable = 'GROUP BY dv.id_prod ORDER BY p.pro_pre ASC';
            }
            $query = "SELECT p.id_areap,p.id_pres,p.pro_nom,p.pro_pre".$campo.",p.pro_img FROM tm_detalle_venta AS dv INNER JOIN v_productos AS p ON dv.id_prod = p.id_pres WHERE p.est_a = 'a' AND p.est_b = 'a' AND p.est_c = 'a' ".$variable;
        ...
        try 
        {
            ...
            foreach($data['items'] as $d)
            {
                ...
                */
                ...
                // ESTE AJUSTE ES PARA QUE NO SALGA MAL LAS COMANDAS. (en caso de revertir el cambio comentar lo de abajo y descomentar lo de arriba.)               
                $id_pedido = $data['cod_ped'];
                $id_presentacion = $d['producto_id'];

                $queryVerificacion = "SELECT count(*) AS total FROM tm_detalle_pedido WHERE estado='a' and id_pedido = :id_pedido AND id_pres = :id_presentacion";
                $result = $this->db->prepare($queryVerificacion);
                $result->bindParam(':id_pedido',$id_pedido,PDO::PARAM_INT);
                $result->bindParam(':id_presentacion',$id_presentacion,PDO::PARAM_STR);
                $result->execute();
                $row = $result->fetch(PDO::FETCH_ASSOC);
                ...
                if ($total_items == 0) {
                    ...
                    $sql = "UPDATE tm_detalle_pedido SET cantidad = cantidad + ?, cant = cant + ? WHERE id_pedido = ? AND id_pres = ?";
                    $this->db->prepare($sql)->execute(array($d['cantidad'],$d['cantidad'],$id_pedido,$id_presentacion));
                }  
            }
        }
        catch (Exception $e) 
    ...
    public function buscar_producto($data)
    {
        ...
        {   
            ...
            $cadena = $data['cadena'];
            $query_buscador = "SELECT id_areap,id_pres,pro_nom,pro_pre".$campo.",pro_img FROM v_productos WHERE (pro_cod LIKE '%$cadena%' OR pro_nom LIKE '%$cadena%' OR pro_pre LIKE '%$cadena%' OR pro_cat LIKE '%$cadena%') AND est_a = 'a' AND est_b = 'a' AND est_c = 'a' ".$variable;
            $stm = $this->db->prepare($query_buscador);
            $stm->execute();
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            foreach($c as $k => $d)
            {
    ...
    */
    ...
    public function RegistrarVenta($data)
    {
        try
        {
            ...
            $dato_pago = $this->db->query("SELECT id_pago FROM tm_tipo_pago WHERE id_tipo_pago = ".$data['tipo_pago'])->fetch(PDO::FETCH_OBJ);
            $id_pago = $dato_pago->id_pago;

            if ($data['tarjetasMontos'][0] != 0) { $pago_mastercard = $data['tarjetasMontos'][0]; }
            if ($data['tarjetasMontos'][1] != 0) { $pago_visa = $data['tarjetasMontos'][1]; }


            if (isset($data['billeterasMontos'][0])) {
                if ($data['billeterasMontos'][0] != 0) { $pago_plin = $data['billeterasMontos'][0]; }
            }
            if (isset($data['billeterasMontos'][1])) {
                if ($data['billeterasMontos'][1] != 0) { $pago_yape = $data['billeterasMontos'][1]; }
            }
            if (isset($data['billeterasMontos'][2])) {
                if ($data['billeterasMontos'][2] != 0) { $pago_lukita = $data['billeterasMontos'][2]; }
            }
            if (isset($data['billeterasMontos'][3])) {
                if ($data['billeterasMontos'][3] != 0) { $pago_tunki = $data['billeterasMontos'][3]; }                
            }

            if ($data['tipo_pago'] == 4) { $pago_lin = $data['total']; }
            if ($data['tipo_pago'] == 7) { $pago_tra = $data['total']; }

            if ($data['tipo_pago'] == 9) { $pago_mastercard = $data['total']; }
            if ($data['tipo_pago'] == 2) { $pago_visa = $data['total']; }

            if ($data['tipo_pago'] == 6) { $pago_lukita = $data['total']; }
            if ($data['tipo_pago'] == 8) { $pago_plin = $data['total']; }
            if ($data['tipo_pago'] == 16) { $pago_tunki = $data['total']; }
            if ($data['tipo_pago'] == 5) { $pago_yape = $data['total']; }
            
            if ($id_pago == 1) {
                #if ($data['pago_efe'] > $data['total']) {
                    $pago_efe = ($data['total'] + $data['comision_delivery']) - $data['descuento_monto'];
                #}
            }

            if ($id_pago == 3) {
                $no_efectivo = $data['pago_tar'] + $data['pago_bil'] + $data['pago_tra'] + $data['pago_lin'];
                if ($no_efectivo < $data['total']) {
                    $pago_efe = (($data['total']  + $data['comision_delivery']) - $data['descuento_monto']) - $no_efectivo;
                }
            }

            if( ($conteo->conteo > 0) &&  $data['total'] > 0){    
                $consulta = "call usp_restEmitirVenta(:flag, :dividir_cuenta, :id_pedido, :tipo_pedido, :tipo_entrega, :id_cliente, :id_tipo_doc, :id_pago, :id_tipo_pago, :id_usu, :id_apc, :pago_efe_none, :pago_efe, :pago_tar, :pago_bil, :pago_tra, :pago_lin, :pago_mastercard, :pago_visa, :pago_lukita, :pago_plin, :pago_tunki, :pago_yape, :descuento_tipo, :descuento_personal, :descuento_monto, :descuento_motivo, :comision_tarjeta, :comision_delivery, :igv, :total, :codigo_operacion, :fecha_venta);";
                $arrayParam = array(
                    ':flag' => 1,
                    ...
                    ':pago_bil' => $data['pago_bil'],
                    ':pago_tra' => $data['pago_tra'],
                    ':pago_lin' => $data['pago_lin'],
                    ':pago_mastercard' => $pago_mastercard,
                    ':pago_visa' => $pago_visa,
                    ':pago_lukita' => $pago_lukita,
                    ':pago_plin' => $pago_plin,
                    ':pago_tunki' => $pago_tunki,
                    ':pago_yape' => $pago_yape,
                ...
                $this->db_consumo = new Database(DB_TYPE, DB_HOST, DB_NAME, DB_USER, DB_PASS, DB_CHARSET);
                $sqlconsumo = "UPDATE tm_venta SET consumo = ?, consumo_desc = ?  WHERE id_venta = ? ";
                $this->db_consumo->prepare($sqlconsumo)->execute(array($data['venta_por_consumo'],$data['venta_por_consumo_concepto'],$id_venta));                
                ...
                for($x=0; $x < sizeof($a); ++$x){
                    if ($b[$x] > 0){
                        $costo_producto = $this->db->query("SELECT precio_compra FROM tm_producto_pres WHERE id_pres = " . $a[$x])->fetch();
                        $costo = $costo_producto['precio_compra'];

                        $sql = "INSERT INTO tm_detalle_venta (id_venta,id_prod,cantidad,precio,costo) VALUES (?,?,?,?,?);";
                        $this->db->prepare($sql)->execute(array($id_venta,$a[$x],$b[$x],$c[$x],$costo));
                    }
                }
    ...
    public function anular_venta($data)
    {
        ...
        {   
            ...
            $sql3 = "UPDATE tm_detalle_pedido SET estado = 'i' WHERE id_pedido = ?";
            $this->db->prepare($sql3)->execute(array($data['id_pedido']));
            $sql4 = "UPDATE tm_pedido SET estado = 'z' WHERE id_pedido = ?";
        ...
        try 
        {
            ...
            foreach($data_producto as $item) {
                ...
                if ($cantidad == 1 OR $cantidad == $cantidad_eliminar) {
                    ...
                    $sql = "UPDATE tm_detalle_pedido SET cantidad = cantidad-" . $cantidad_eliminar . " WHERE estado='a' and id_pedido = " . $data['id_pedido'] . " AND id_pres = " . $data['id_pres'] . "";
                    $stm = $this->db->prepare($sql);
                    $stm->execute();                    

                    $sql = "UPDATE tm_detalle_pedido SET cant = cant-" . $cantidad_eliminar . " WHERE estado='a' and id_pedido = " . $data['id_pedido'] . " AND id_pres = " . $data['id_pres'] . "";
                    $stm = $this->db->prepare($sql);
                    $stm->execute();                    

                    $codigo_respuesta = array(array('cod'=>'1'));
                }
            }
    ...
    public function venta_edit_pago($data)
    {
        try 
        {
			...
			error_log("tipo pago => " . $data['tipo_pago']);
			error_log("id_tipo_pago => " . $data['id_tipo_pago']);
            ...
            if($data['tipo_pago'] <> $data['id_tipo_pago']){
                if($data['tipo_pago'] == 1){
                    ...
                    if($data['id_tipo_pago'] == 1){
                        $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_efe = ?, pago_efe_none = '0.00', pago_tar = '0.00', pago_bil = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                        $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$data['id_venta']));
                    } else {
            ...
            */
            ...
            $ds = $this->db->prepare("SELECT (pago_efe+pago_tar+pago_bil+pago_tra+pago_lin) as total FROM tm_venta WHERE id_venta=" . $data['id_venta']);
			$ds->execute();
			$data_s = $ds->fetch();
			$monto_global = $data_s['total'];

            if ($data['id_tipo_pago'] == 1) { // Efectivo
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_efe = ?, pago_tar = '0.00', pago_bil = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$data['id_venta']));
            }            
            if ($data['id_tipo_pago'] == 2) { // Visa
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_tar = ?, pago_visa = ?, pago_efe = '0.00', pago_bil = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 4) { // Culqi
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_lin = ?, pago_efe = '0.00', pago_tar = '0.00', pago_bil = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00',  pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 5) { // Yape
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_bil = ?, pago_yape = ?, pago_efe = '0.00', pago_tar = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 6) { // Lukita
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_bil = ?, pago_lukita = ?, pago_efe = '0.00', pago_tar = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 7) { // Transferencia
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_tra = ?, pago_efe = '0.00', pago_tar = '0.00', pago_bil = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 8) { // Plin
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_bil = ?, pago_plin = ?, pago_efe = '0.00', pago_tar = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_mastercard = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 9) { // Mastercard
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_tar = ?, pago_mastercard = ?, pago_efe = '0.00', pago_bil = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_tunki = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$monto_global,$data['id_venta']));
            }
            if ($data['id_tipo_pago'] == 16) { // Tunki
                $sql = "UPDATE tm_venta SET id_tipo_pago = ?, pago_bil = ?, pago_tunki = ?, pago_efe = '0.00', pago_tar = '0.00', pago_tra = '0.00', pago_lin = '0.00', pago_visa = '0.00', pago_lukita = '0.00', pago_plin = '0.00', pago_mastercard = '0.00', pago_yape = '0.00' WHERE id_venta = ?";
                $this->db->prepare($sql)->execute(array($data['id_tipo_pago'],$monto_global,$monto_global,$data['id_venta']));
            }            
...
public function combo_productos($data)
{
    ...
    {   
        ...
        $validar =  $this->db->selectAll('SELECT * FROM v_productos WHERE id_pres = '.$data['id_prod'].' AND id_catg = 1');
