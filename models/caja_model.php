...
class Caja_Model extends Model
{
    ...
    public function apercie_listcaja()
    {
        try
        {
            $stm = $this->db->prepare("SELECT tac.* FROM tm_caja AS ca INNER JOIN v_caja_aper AS tac ON ca.id_caja = tac.id_caja WHERE ca.estado ='a' AND tac.estado = 'a';");
            $stm->execute(array());
    ...
    public function monitor_ventas_list()
    {
        try
        {
            ...
            $stm = $this->db->prepare("SELECT *,IFNULL((pago_efe+pago_tar+pago_bil+pago_tra+pago_lin),0) AS monto_total, total AS monto_neto,(total + comis_del) AS total_completo FROM  v_ventas_con WHERE (DATE_FORMAT(fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(fec_ven,'%Y-%m-%d') <= ?) AND id_tped like ? AND id_tdoc like ? AND estado like ? ".$consulta."");
            
            $stm->execute(array($ifecha,$ffecha,$_POST['tped'],$_POST['tdoc'],$_POST['estado']));
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
