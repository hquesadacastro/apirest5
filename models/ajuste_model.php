...
class Ajuste_Model extends Model
{
    ...
    public function Permisos()
    {
        try
        {      
            return $this->db->selectAll('SELECT * FROM tm_permisos');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function usuario_recuperar_permisos($id)
    {
        $link = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME);

        $permisos = array();

        $sql = "SELECT id_permiso FROM tm_permisos_asignados WHERE id_usuario = '$id'";
        $rs = mysqli_query($link,$sql) or die("ID USUARIO : " . $sql . mysqli_error($link));
    ...
    public function usuario_guardar_permisos($usuario, $permisos)
    {
        $link = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME);

        $sql = "SELECT id_usu FROM tm_usuario WHERE usuario = '$usuario'";
        $rs = mysqli_query($link,$sql) or die("ID USUARIO : " . $sql . mysqli_error($link));
        while($row = mysqli_fetch_array($rs)) {
        ...
        $sql = "DELETE FROM tm_permisos_asignados WHERE id_usuario='$id_usuario'";
        $rs = mysqli_query($link,$sql);

        if (gettype($permisos) == "array") {
            foreach($permisos as $permiso) {
                $sql = "INSERT INTO tm_permisos_asignados (id_usuario,id_permiso) VALUES ('$id_usuario','$permiso')";
                $rs = mysqli_query($link,$sql);
            }            
        }
    ...
    public function usuario_crud_create($data)
    {
        try 
        {
            ...
            if (isset($data['permisosCajero'])) {
                if ($data['id_rol'] == '3') {
                    $permisos = $data['permisosCajero'];
                }    
            }

            if (isset($data['permisosMozo'])) {
                if ($data['id_rol'] == '5') {
                    $permisos = $data['permisosMozo'];
                }    
            }

            $usuario = $data['usuario'];
            $this->usuario_guardar_permisos($usuario, $permisos);
    ...
    public function usuario_delete($data)
    {
        try 
        {
        $stm = $this->db->prepare("DELETE FROM tm_permisos_asignados WHERE id_usuario = ?");          
    ...
    public function TipoPago()
    {
        try
        {      
            return $this->db->selectAll('SELECT * FROM tm_pago WHERE (id_pago!=1 AND id_pago!=3)');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function tipopago_list($data)
    {
        try
        {
            $stm = $this->db->prepare("SELECT * FROM tm_tipo_pago WHERE id_tipo_pago LIKE ? AND (id_tipo_pago != 1 AND id_tipo_pago != 3) ORDER BY id_pago asc, descripcion asc");
    ...
    */
    ...
    public function producto_pres_crud_create($data)
    {
        try 
        {
            ...
            //$data['hidden_stock_limit'] = 99999;
            if (isset($data['producto_transformado'])) {
                if ($data['producto_transformado'] == 2) {
                    $valor_receta = 0;
                } else {
                    $valor_receta = $data['hidden_receta_presentacion'];
                }    
            } else {
                $valor_receta = 0;
            }

            $consulta = "call usp_configProductoPres( :flag, @a, :id_prod, :cod_prod, :presentacion, :descripcion, :precio, :precio_compra, :precio_delivery, :receta, :stock_min,:stock_limit, :impuesto, :delivery, :margen, :igv, :imagen, :estado);";
            $arrayParam =  array(
                ':flag' => 1,
    ...
    public function producto_pres_crud_update($data)
    {
        try 
        {
            ...
            $consulta = "call usp_configProductoPres( :flag, :id_pres, :id_prod, :cod_prod, :presentacion, :descripcion, :precio, :precio_compra, :precio_delivery, :receta, :stock_min,:stock_limit, :impuesto, :delivery, :margen, :igv, :imagen, :estado);";
            $arrayParam =  array(
                ':flag' => 2,
    ...
    */
    ...
    public function optimizar_kardex_inventario()
    {
        try
        {
            $consulta = "call usp_optPedidos(:flag);";
    ...
    public function datosistema_crud($data)
    {
        try 
        {
            //$sql = "UPDATE tm_configuracion SET zona_hora = ?,trib_acr = ?,trib_car = ?,di_acr = ?,di_car = ?,imp_acr = ?,imp_val = ?,mon_acr = ?,mon_val = ?,pc_name = ?,pc_ip = ?,print_com = ?,print_pre = ?,print_cpe = ?,cod_seg = ?, opc_01 = ?, com_tar = ?, com_tar_val = ?, note_ticket = ?, nota_ind = ?, print_com_combo = ?";
            //$this->db->prepare($sql)->execute(array($data['zona_hora'],$data['trib_acr'],$data['trib_car'],$data['di_acr'],$data['di_car'],$data['imp_acr'],$data['imp_val'],$data['mon_acr'],$data['mon_val'],$data['pc_name'],$data['pc_ip'],$data['print_com'],$data['print_pre'],$data['print_cpe'],$data['cod_seg'],$data['opc_01'],$data['com_tar'],$data['com_tar_val'],$data['note_ticket'],$data['nota_ind'], $data['print_com_combo']));

            $sql = "UPDATE tm_configuracion SET zona_hora = ?,trib_acr = ?,trib_car = ?,di_acr = ?,di_car = ?,imp_acr = ?,imp_val = ?,mon_acr = ?,mon_val = ?,pc_name = ?,pc_ip = ?,print_com = ?,print_pre = ?,print_cpe = ?,cod_seg = ?,cod_admin = ?, opc_01 = ?, print_com_combo = ?";
            $this->db->prepare($sql)->execute(array($data['zona_hora'],$data['trib_acr'],$data['trib_car'],$data['di_acr'],$data['di_car'],$data['imp_acr'],$data['imp_val'],$data['mon_acr'],$data['mon_val'],$data['pc_name'],$data['pc_ip'],$data['print_com'],$data['print_pre'],$data['print_cpe'],$data['cod_seg'],$data['cod_admin'],$data['opc_01'],$data['print_com_combo']));
            ...
            Session::set('cod_admin', $data['cod_admin']); //funcion codigo de administrador
                ...
                if($up->uploaded){
                    ...
                    if($up->processed){
                        ...
                        for ($row = 2; $row <= $highestRowp; $row++){ 
                            ...
                            $p_precio_compra           = $sheetpre->getCell("F".$row)->getValue();
                            $p_precio_delivery  = $sheetpre->getCell("G".$row)->getValue();
                            $p_stock_min        = $sheetpre->getCell("H".$row)->getValue();
                            $p_control_stock    = $sheetpre->getCell("I".$row)->getValue();
                            $p_igv              = $sheetpre->getCell("J".$row)->getValue();
                            $p_delivery         = $sheetpre->getCell("K".$row)->getValue();
                            // REMPLAZAR 
                            ...
                            if ($sql_producto) {
                                ...
                                if(!$sql_presentacion){
                                    $insert_presentacion = "INSERT INTO tm_producto_pres SET id_prod = ?,cod_prod = ?,presentacion = ?,descripcion = ?,precio = ?,precio_compra = ?,precio_delivery = ?,receta = ?,stock_min = ?,crt_stock = ?,impuesto = ?,delivery = ?,margen = ?,igv = ?";
                                    $result_presentacion = $this->db->prepare($insert_presentacion)->execute(array($id_p_producto, $p_cod_presentacion, $p_presentacion, $p_descripcion, $p_precio, $p_precio_compra, $p_precio_delivery, '0',$p_stock_min,$p_control_stock,$p_igv, $p_delivery,'0', $p_igv_monto)); 
                                } 
                            }
                        }
