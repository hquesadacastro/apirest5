...
class Informe_Model extends Model
{
    ...
    public function Salon()
    {
        try
        {      
            return $this->db->selectAll("SELECT id_salon, descripcion AS nombre FROM tm_salon WHERE estado = 'a'");
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function Mesa()
    {
        try
        {      
            return $this->db->selectAll("SELECT id_mesa, nro_mesa AS nombre FROM tm_mesa WHERE estado = 'a'");
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function Pago()
    {
        try
        {   
            return $this->db->selectAll('SELECT * FROM tm_pago ORDER BY id_pago ASC, descripcion ASC');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function TipoPago()
    {
        try
        {   
            return $this->db->selectAll('SELECT * FROM tm_tipo_pago WHERE estado = "a" and id_tipo_pago!=3 ORDER BY id_pago ASC, descripcion ASC');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function venta_all_list()
    {
        ...
        {   
            ...
            $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,(v.pago_efe + v.pago_tar + v.pago_bil + v.pago_tra + v.pago_lin) AS total_completo,v.fec_ven,v.desc_td,v.ser_doc,v.nro_doc,v.estado,CASE WHEN v.estado = 'a' THEN v.pago_efe + v.pago_tar ELSE 0 END AS total,v.id_cli,v.igv,v.id_usu,v.desc_tipo,v.desc_personal,c.desc_caja, c.id_apc FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.id_tped like ? AND v.id_tdoc like ? AND v.id_cli like ? AND v.estado like ? GROUP BY v.id_ven");
    ...
    public function venta_all_det($data)
    {
        try
        {
            ...
            foreach($c as $k => $d)
            {
                ...
                $c[$k]->{'TotalVenta'} = $this->db->query("SELECT SUM(comision_delivery + total) AS total FROM tm_venta WHERE id_venta = ".$data['id_venta'])
                ->fetch(PDO::FETCH_OBJ);
            }
            return $c;
        }
    ...
    public function venta_prod_kardex_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d',strtotime($data['ffecha']));
            $query = $this->db->prepare("SELECT v.id_ven,v.fec_ven AS fecha_venta, v.desc_td AS documento_venta, CONCAT(v.ser_doc,'-',v.nro_doc) AS nro_documento, CONCAT(p.pro_nom,' - ',p.pro_pre) AS producto_presentacion, p.pro_cat AS producto_categoria, dv.cantidad AS cantidad_vendida, dv.precio AS precio_venta, (dv.cantidad*dv.precio) AS total
    ...
    public function venta_prod_margen_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d',strtotime($data['ffecha']));
            $stm = $this->db->prepare("SELECT CONCAT(pp.presentacion, ' ', p.nombre) AS producto_presentacion, pc.descripcion AS producto_categoria, SUM(dv.cantidad) AS cantidad_vendida, dv.costo AS costo_unitario, (SUM(dv.cantidad) * dv.costo) AS costo_total, dv.precio AS precio_venta, (dv.precio - dv.costo) AS margen_unitario, (SUM(dv.cantidad) * (dv.precio - dv.costo)) AS margen_total, (SUM(dv.cantidad) * dv.precio) AS total FROM tm_detalle_venta AS dv LEFT JOIN tm_venta AS v ON v.id_venta=dv.id_venta LEFT JOIN tm_producto_pres AS pp ON pp.id_pres=dv.id_prod LEFT JOIN tm_producto AS p ON p.id_prod=pp.id_prod LEFT JOIN tm_producto_catg AS pc ON pc.id_catg=p.id_catg WHERE (DATE(v.fecha_venta) >= ? AND DATE(v.fecha_venta) <= ?) AND p.id_catg LIKE ? AND p.id_prod LIKE ? AND pp.id_pres LIKE ? AND v.estado = 'a' GROUP BY dv.id_prod, dv.costo");
            $stm->execute(array($ifecha,$ffecha,$data['id_catg'],$data['id_prod'],$data['id_pres']));
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
    ...
    public function venta_credit_per_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d',strtotime($data['ffecha']));
            $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.desc_monto,v.desc_tipo,v.desc_motivo,v.comis_tar,v.comis_del,v.total AS stotal,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.desc_monto),0) AS total_descuento, v.total ,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.desc_usu FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE(v.fec_ven) >= ? AND DATE(v.fec_ven) <= ?) AND v.estado = 'a' AND v.desc_tipo = 3 GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
            $stm->execute(array($ifecha,$ffecha));
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            $data = array("data" => $c);
            $json = json_encode($data);
    ...
    public function venta_fpago_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d',strtotime($data['ffecha']));
            $id_tipo_pago = $data['id_tpag'];

            if ($id_tipo_pago == '%') {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar+v.pago_bil+v.pago_tra+v.pago_lin),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.id_tpag LIKE ? AND v.estado = 'a' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha,$data['id_tpag']));    
            }

            if ($id_tipo_pago == 1) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar+v.pago_bil+v.pago_tra+v.pago_lin),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.id_tpag LIKE ? AND v.estado = 'a' OR v.pago_efe!=0 GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha,$data['id_tpag']));    
            }            

            if ($id_tipo_pago == 9) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_mastercard>0 AND v.estado = 'a' OR v.id_tpag ='9' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 2) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_visa>0 AND v.estado = 'a' OR v.id_tpag ='2' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 4) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_lin>0 AND v.estado = 'a' OR v.id_tpag ='4' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 7) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_tra>0 AND v.estado = 'a' OR v.id_tpag ='7' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 6) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_lukita>0 AND v.estado = 'a' OR v.id_tpag ='6' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 8) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_plin>0 AND v.estado = 'a' OR v.id_tpag ='8' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 16) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_tunki>0 AND v.estado = 'a' OR v.id_tpag ='16' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }

            if ($id_tipo_pago == 5) {
                $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tpag,v.pago_efe,v.pago_tar,v.pago_bil,v.pago_tra,v.pago_lin,v.pago_mastercard,v.pago_visa,v.pago_lukita,v.pago_plin,v.pago_tunki,v.pago_yape,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.total_completo AS total_completo,v.fec_ven,v.desc_td,CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total,v.id_cli,v.igv,v.id_usu,c.desc_caja,v.estado,v.codigo_operacion FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.pago_yape>0 AND v.estado = 'a' OR v.id_tpag ='5' GROUP BY v.id_ven ORDER BY DATE(v.fec_ven) ASC");
                $stm->execute(array($ifecha,$ffecha));    
            }
    ...
    public function venta_all_imp($data)
    {
        ...
        {               
            ...
            $c->{'Empresa'} = $this->db->query("SELECT * FROM tm_empresa")->fetch(PDO::FETCH_OBJ);
            ...
            $c->{'Detalle'} = $this->db->query("SELECT v_productos.pro_cod AS codigo_producto, 
                ...
                IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ".igv_dec."))*tm_detalle_venta.cantidad,2),
                ROUND(tm_detalle_venta.precio*tm_detalle_venta.cantidad,2)) AS valor_venta,
                IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ".igv_dec.")*tm_detalle_venta.cantidad)*".igv_dec.",2),0) AS total_igv 
                ...
                WHERE tm_venta.id_tipo_doc  IN ('1','2','3') AND tm_detalle_venta.id_venta = ".$data . " order by tm_detalle_venta.secuencia ASC")
    ...
    public function compra_all_det($data)
    {
        try
        {
            ...
            foreach($c as $k => $d)
            {
                ...
                $c[$k]->{'Compra'} = $this->db->query("SELECT total, descuento, adicional FROM tm_compra WHERE id_compra = ".$d->id_compra)
                    ->fetch(PDO::FETCH_OBJ);                    
            }
            return $c;
        }
    ...
    public function finanza_arq_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d',strtotime($data['ffecha']));
            $stm = $this->db->prepare("SELECT * FROM v_caja_aper WHERE (DATE_FORMAT(fecha_aper,'%Y-%m-%d') >= ? AND DATE_FORMAT(fecha_aper,'%Y-%m-%d') <= ?) AND id_usu like ? ORDER BY id_apc DESC");
    ...
    public function finanza_arq_resumen_default($data)
    {
        try
        {    
            $stm = $this->db->prepare("SELECT v.id_apc,v.id_ped,IFNULL(SUM(v.pago_efe),0) AS pago_efe, IFNULL(SUM(v.pago_tar),0) AS pago_tar, IFNULL(SUM(v.pago_yape),0) AS pago_yape, IFNULL(SUM(v.pago_plin),0) AS pago_plin, IFNULL(SUM(v.pago_tra),0) AS pago_tra, IFNULL(SUM(v.pago_lin),0) AS pago_lin, IFNULL(SUM(v.desc_monto),0) AS descu, IFNULL(SUM(v.comis_tar),0) AS comis_tar, IFNULL(SUM(v.comis_del),0) AS comis_del, IFNULL(SUM(v.pago_efe+v.pago_tar+v.pago_bil+v.pago_tra+v.pago_lin),0) AS total, v.estado FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = ? AND v.estado <> 'i' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 8000)");
    ...
    public function finanza_arq_resumen_venta_list($data)
    {
        ...
        {   
            if($data['cod_filtro'] == 1){
                $stm = $this->db->prepare("SELECT IFNULL((v.pago_efe+v.pago_tar+v.pago_bil+v.pago_tra+v.pago_lin),0) AS monto_total,v.estado,v.ser_doc,v.nro_doc,v.desc_td,v.desc_monto FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = ? AND v.estado = ? AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 8000)");
            } else {
                $stm = $this->db->prepare("SELECT IFNULL((v.pago_efe+v.pago_tar+v.pago_bil+v.pago_tra+v.pago_lin),0) AS monto_total,v.estado,v.ser_doc,v.nro_doc,v.desc_td,v.desc_monto FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = ? AND v.estado = ? AND v.desc_monto <> '0.00' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 8000)");
            }
    ...
    public function comanda_mesero_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d H:i:s',strtotime($data['ifecha']));
            ...
            $stm = $this->db->prepare("SELECT v.id_venta, v.fecha_venta, pp.id_pres, pm.id_mesa, pm.id_mozo, m.id_salon, dp.cant AS cantidad FROM tm_venta AS v LEFT JOIN tm_detalle_pedido AS dp ON dp.id_pedido=v.id_pedido LEFT JOIN tm_producto_pres AS pp ON pp.id_pres=dp.id_pres LEFT JOIN tm_producto AS p ON p.id_prod=pp.id_prod LEFT JOIN tm_pedido_mesa AS pm ON pm.id_pedido=v.id_pedido LEFT JOIN tm_usuario AS u ON u.id_usu=pm.id_mozo LEFT JOIN tm_mesa AS m ON m.id_mesa=pm.id_mesa LEFT JOIN tm_salon AS s ON s.id_salon=m.id_salon WHERE (DATE(v.fecha_venta) >= ? AND DATE(v.fecha_venta) <= ?) AND pm.id_mozo LIKE ? AND m.id_salon LIKE ? AND pm.id_mesa LIKE ? AND v.estado='a' ORDER BY v.fecha_venta DESC");
            $stm->execute(array($ifecha,$ffecha,$data['id_mozo'],$data['id_salon'],$data['id_mesa']));
            $c = $stm->fetchAll(PDO::FETCH_OBJ);        
            foreach($c as $k => $d)
            {
                $c[$k]->{'Mozo'} = $this->db->query("SELECT CONCAT(nombres,' ',ape_paterno,' ',ape_materno) AS nombre FROM v_usuarios WHERE id_usu = ".$d->id_mozo)
                ...
                $c[$k]->{'Salon'} = $this->db->query("SELECT descripcion AS nombre FROM tm_salon WHERE id_salon = ".$d->id_salon)
                    ->fetch(PDO::FETCH_OBJ);

                $c[$k]->{'Mesa'} = $this->db->query("SELECT nro_mesa AS nombre FROM tm_mesa WHERE id_mesa = ".$d->id_mesa)
                    ->fetch(PDO::FETCH_OBJ); 

                $c[$k]->{'Producto'} = $this->db->query("SELECT pc.descripcion AS categoria, pp.presentacion, p.nombre AS producto FROM tm_producto_pres AS pp LEFT JOIN tm_producto AS p ON p.id_prod=pp.id_prod LEFT JOIN tm_producto_catg AS pc ON pc.id_catg=p.id_catg WHERE pp.id_pres=".$d->id_pres)
                    ->fetch(PDO::FETCH_OBJ);
            }
            $data = array("data" => $c);
            $json = json_encode($data);
    ...
    public function finanza_arq_imp($data)
    {
        ...
        {      
            ...
            $c->{'Principal'} = $this->db->query("SELECT v.id_apc,v.id_ped,IFNULL(SUM(v.pago_efe),0) AS pago_efe, IFNULL(SUM(v.pago_tar),0) AS pago_tar, IFNULL(SUM(v.desc_monto),0) AS descu, IFNULL(SUM(v.comis_tar),0) AS comis_tar, IFNULL(SUM(v.comis_del),0) AS comis_del, IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total, IFNULL(SUM(v.total),0) AS gran_total, v.estado FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 8000)")
            ->fetch(PDO::FETCH_OBJ);

            $c->{'Efectivo'} = $this->db->query("SELECT IF(v.id_tpag = 1 OR v.id_tpag = 3,SUM(v.pago_efe),0) AS total, COUNT(*) AS cant FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 8000) AND (v.id_tpag = 1 OR v.id_tpag = 3)")
            ->fetch(PDO::FETCH_OBJ);

            $c->{'Ventas_Efectivo'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_efe) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='1' OR v.id_tipo_pago='3')")
            ->fetchAll(PDO::FETCH_OBJ); 

            $c->{'Ventas_VISA'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_visa) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='2' OR v.id_tipo_pago='3') AND v.pago_visa>0")
            ->fetchAll(PDO::FETCH_OBJ); 

            $c->{'Ventas_Mastercard'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_mastercard) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='9' OR v.id_tipo_pago='3') AND v.pago_mastercard>0")
            ->fetchAll(PDO::FETCH_OBJ);

            $c->{'Ventas_CULQI'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_lin) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='4' OR v.id_tipo_pago='3') AND v.pago_lin>0")
            ->fetchAll(PDO::FETCH_OBJ); 

            $c->{'Ventas_YAPE'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_yape) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='5' OR v.id_tipo_pago='3') AND v.pago_yape>0")
            ->fetchAll(PDO::FETCH_OBJ);   

            $c->{'Ventas_LUKITA'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_lukita) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='6' OR v.id_tipo_pago='3') AND v.pago_lukita>0")
            ->fetchAll(PDO::FETCH_OBJ);  

            $c->{'Ventas_Transferencia'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_tra) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='7' OR v.id_tipo_pago='3') AND v.pago_tra>0")
            ->fetchAll(PDO::FETCH_OBJ);  

            $c->{'Ventas_PLIN'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_plin) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='8' OR v.id_tipo_pago='3') AND v.pago_plin>0")
            ->fetchAll(PDO::FETCH_OBJ);  

            $c->{'Ventas_TUNKI'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_tunki) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (v.id_tipo_pago='16' OR v.id_tipo_pago='3') AND v.pago_tunki>0")
            ->fetchAll(PDO::FETCH_OBJ);  

            /*
            $c->{'Ventas_IZIPAY'} = $this->db->query("SELECT tp.descripcion, count(*) AS cantidad, SUM(v.pago_tar) AS monto FROM tm_venta AS v LEFT JOIN tm_tipo_pago AS tp ON tp.id_tipo_pago=v.id_tipo_pago WHERE v.id_tipo_pago='19'")
            ->fetchAll(PDO::FETCH_OBJ); 
            */     

            $c->{'Pagos'} = $this->db->query("SELECT a.id_pago, a.id_tipo_pago, b.DESCRIPCION, 
            ...
            WHERE a.id_tipo_pago = b.id_tipo_pago AND a.id_tipo_pago != 3 AND a.id_apc = {$data} and a.estado!='i' GROUP BY a.id_tipo_pago")
            ->fetchAll(PDO::FETCH_OBJ);                
            ...
            $c->{'PedidosYa'} = $this->db->query("SELECT IFNULL(SUM(v.pago_efe + v.pago_tar),0) AS total, COUNT(*) AS cant FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$data} AND v.estado <> 'i' AND d.id_repartidor = 3333")
            ->fetch(PDO::FETCH_OBJ);

            $c->{'Ingresos'} = $this->db->query("SELECT IFNULL(SUM(importe),0) AS total FROM tm_ingresos_adm WHERE id_apc = {$data} AND estado='a'")
            ->fetch(PDO::FETCH_OBJ);
            
            $c->{'Egresos'} = $this->db->query("SELECT IFNULL(SUM(importe),0) AS total FROM v_gastosadm WHERE id_apc = {$data} AND (id_tg = 1 OR id_tg = 2 OR id_tg = 3 OR id_tg = 4 OR id_tg = 5) AND estado='a'")
            ->fetch(PDO::FETCH_OBJ);
            ...
            $c->{'EgresosE'} = $this->db->query("SELECT IFNULL(SUM(importe),0) AS total FROM v_gastosadm WHERE id_apc = {$data} AND id_tg = 5 AND estado='a'")
            ->fetch(PDO::FETCH_OBJ);            

            $c->{'Descuentos'} = $this->db->query("SELECT COUNT(id_ven) AS cant, SUM(total) AS total FROM v_ventas_con WHERE id_apc = {$data} AND estado <> 'i' AND desc_tipo='1'")
            ->fetch(PDO::FETCH_OBJ);

            $c->{'Cortesia'} = $this->db->query("SELECT COUNT(id_ven) AS cant, SUM(total) AS total FROM v_ventas_con WHERE id_apc = {$data} AND estado <> 'i' AND desc_tipo='1'")
            ->fetch(PDO::FETCH_OBJ);   

            $c->{'Descuento'} = $this->db->query("SELECT COUNT(id_ven) AS cant, SUM(desc_monto) AS total FROM v_ventas_con WHERE id_apc = {$data} AND estado <> 'i' AND desc_tipo='2' AND desc_monto>0.00")
            ->fetch(PDO::FETCH_OBJ); 

            $c->{'CreditoPersonal'} = $this->db->query("SELECT COUNT(id_ven) AS cant, SUM(desc_monto) AS total FROM v_ventas_con WHERE id_apc = {$data} AND estado <> 'i' AND desc_tipo='3' AND desc_monto>0.00")
            ->fetch(PDO::FETCH_OBJ);             
            ...
            $c->{'Anulaciones'} = $this->db->query("SELECT COUNT(*) AS cant, SUM(pago_efe + pago_tar + pago_bil + pago_tra + pago_lin) total FROM tm_venta WHERE estado = 'i' AND id_apc = {$data}")
            ->fetch(PDO::FETCH_OBJ);

            $c->{'Deliverys'} = $this->db->query("SELECT SUM(IFNULL((v.pago_efe + v.pago_tar + v.pago_bil + v.pago_tra + v.pago_lin),0)) AS total, COUNT(*) AS cant FROM v_ventas_con AS v INNER JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$data} AND v.estado = 'a'")
            ->fetch(PDO::FETCH_OBJ);
    ...
    public function finanza_arq_resumen_imp($data)
    {
        ...
        {      
            ...
            $c->{'Principal'} = $this->db->query("SELECT v.id_apc,v.id_ped,IFNULL(SUM(v.pago_efe),0) AS pago_efe, IFNULL(SUM(v.pago_tar),0) AS pago_tar, IFNULL(SUM(v.desc_monto),0) AS descu, IFNULL(SUM(v.comis_tar),0) AS comis_tar, IFNULL(SUM(v.comis_del),0) AS comis_del, IFNULL(SUM(v.pago_efe+v.pago_tar),0) AS total, v.estado FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$data} AND v.estado <> 'i' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 8000)")
            ->fetch(PDO::FETCH_OBJ);
