...
class Venta extends Controller {
    ...
    function impresion_precuenta($id_pedido)
    {
     ...
     //AQUI EMPIEZA EL CODIGO PARA PRECUENTA ADICIONAL (CONSIDERAR PARA CADA PRECUENTA PONER EL IP DE LA PC DONDE SERA LA PRECUENTA 2 Y QUE LA PC TENGA EL XAMPP Y LA CARPETA IMPRIMIR)
     //SI SE DESEA MAS PRECUENTAS CONSIDERAR EL IP DE CADA UNO.

     function impresion_precuenta_opcional($id_pedido)
     {
         if(Session::get('print_pre') == 1){
             $dato = $this->model->impresion_precuenta($id_pedido);
             header('location: http://192.168.0.11/imprimir/pre_cuenta1.php?data='.urlencode(json_encode($dato)));
         } else {
             $this->view->dato = $this->model->impresion_precuenta($id_pedido);
             $this->view->render('venta/imprimir/precuenta', true);
         }
     }
     function impresion_precuenta_opcional_dos($id_pedido)
     {
         if(Session::get('print_pre') == 1){
             $dato = $this->model->impresion_precuenta($id_pedido);
             header('location: http://192.168.0.8/imprimir/pre_cuenta2.php?data='.urlencode(json_encode($dato)));
         } else {
             $this->view->dato = $this->model->impresion_precuenta($id_pedido);
             $this->view->render('venta/imprimir/precuenta', true);
         }
     }
     //AQUI FINALIZA EL CODIGO PARA PRECUENTA ADICIONALES
    // pdf reparto 
