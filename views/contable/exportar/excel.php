...
<table id="bootstrap-table">
    <tr>
        <td colspan="<?php print $col_span; ?>" class="text-center font-weight"> RAZON SOCIAL: <?php print(utf8_decode($this->Empresa['razon_social'])); ?></td>
    </tr>
    <tr>
        <td colspan="<?php print $col_span; ?>" class="text-center font-weight"> NOMBRE COMERCIAL: <?php print(utf8_decode($this->Empresa['nombre_comercial'])); ?></td>
    </tr>
    <tr>
        <td colspan="<?php print $col_span; ?>" class="text-center font-weight"> RUC: <?php print(utf8_decode($this->Empresa['ruc'])); ?></td>
    </tr>
    <tr>
    ...
    ?>
     <tr>
            ...
            if($row->desc_monto > 0){
                $desc_base   =  round(($row->desc_monto / igv_dec2), 2);
                $desc_igv    = round(($row->desc_monto - $desc_base), 2);
                $desc_exo   =  round(($row->desc_monto), 2);
            }
            if($row->comis_del > 0){
                $del_base   =  round(($row->comis_del / igv_dec2), 2);
                $del_igv    = round(($row->comis_del - $del_base), 2);
                $del_exo   =  round(($row->comis_del), 2);
            }
            ...
            if ($row->estado == 'i') : 
                ...
                $total_taxed = ($total_taxed > 0)? ($total_taxed + $del_base) - $desc_base  : $total_taxed;

                $total_exonerated = $total_ope_exoneradas;
                $total_exonerated = ($total_exonerated > 0)? ($total_exonerated + $del_exo)- $desc_exo : $total_exonerated; 

                $total_igv = $total_igv_gravadas;
                $total_igv = ($total_igv > 0)? ($total_igv + $del_igv) - $desc_igv : $total_igv;
            ...
            <!-- @else -->
                ...
                <td></td>
                <td></td>
                <td><?php print $estado_comprobante ; ?></td>
            <!-- @endif -->
        </tr>
    <?php }?>
</table>