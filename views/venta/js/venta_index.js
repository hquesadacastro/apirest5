            ...
            $.each(item['mesa'], function(i, mesa) {
                ...
                if($('#rol_usr').val() == 5){
                    ...
                    if(mesa.id_salon == cod && mesa.estado == 'a'){
                        $('.list-mesas')
                            .append($('<li><button class="btn btn-outline-success dim btn-large-dim"'
                                +'onclick="registrarMesa('+mesa.id_mesa+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\');">'+mesa.nro_mesa+'<input type="radio" autocomplete="off"></button></li>'));
                    } else if(mesa.id_salon == cod && mesa.estado == 'p'){
                        $('.list-mesas')
                            $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<li><button class="btn btn-info dim btn-large-dim"'
                                +'onclick="orden('+mesa.id_pedido+')">'
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span>'
                                +'<span class="span-b"><i class="ti-timer"></i>&nbsp;<span class="hora-mesa'+count+++'">'+moment(mesa.fecha_pedido).fromNow(true)+'</span></span><input type="radio" autocomplete="off"></button></li>'));
                    } else if(mesa.id_salon == cod && mesa.estado == 'i'){
                        $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<li><button class="btn btn-reed dim btn-large-dim"'
                                +'onclick="orden('+mesa.id_pedido+')">'
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span>'
                                +'<span class="span-b"><i class="ti-timer"></i>&nbsp;<span class="hora-mesa'+count+++'">'+moment(mesa.fecha_pedido).fromNow(true)+'</span></span><input type="radio" autocomplete="off"></button></li>'));
                    }
                    ...
                    if(mesa.id_salon == cod && mesa.estado == 'a'){
                        $('.list-mesas')
                            .append($('<li><button class="btn btn-outline-success dim btn-large-dim"'
                                +'onclick="nuevoPedidoMesa('+mesa.id_mesa+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'+mesa.nro_mesa+'<input type="radio" autocomplete="off"></button></li>'));
                    } else if(mesa.id_salon == cod && mesa.estado == 'p'){
                        $('.list-mesas')
                            $('.list-mesas')
                            .append($('<li><input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<button class="btn btn-info dim btn-large-dim" onclick="listarPedidos('+mesa.id_salon+','+mesa.id_mesa+',1,'+mesa.id_pedido+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span>'
                        ...
                        $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<li><button class="btn btn-reed dim btn-large-dim" onclick="listarPedidos('+mesa.id_salon+','+mesa.id_mesa+',1,'+mesa.id_pedido+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'
...
var nuevoPedidoMesa = function(id_mesa,mesa,salon){
    ...
    $('.numero-personas').TouchSpin({
        buttondown_class: "btn btn-link text-success bt-down",
        buttonup_class: "btn btn-link text-success bt-up",
        min: 1,
        mousewheel: false
    });
    ///////////////////////////////////////////   
