...
var listarCategorias = function(){
    ...
    $.ajax({
        ...
        success: function(item){
           item.data.length > 0 ? $.each(item.data, function(i, campo) {
                $("#ul-cat").append($('<li class="row my-1"/>').html('<div class="col-auto mr-auto"><a href="javascript:void(0)" class="link-2" onclick="listarProductos(' + campo.id_catg + ')">' + campo.descripcion + '</a></div><div class="col-auto"><div class="dropdown-action"><div class="dropdown todo-action-dropdown"><button class="btn btn-sm btn-outline-primary dropdown-toggle text-decoration-none todo-action-dropdown" type="button"id="more-action-3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="icon-options-vertical"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="more-action-3" style="overflow: auto;"><li><a class="edit dropdown-item pl-2" href="javascript:void(0);" onclick="editarCategoria(' + campo.id_catg + ",'" + campo.descripcion + "'," + campo.delivery + "," + campo.orden + ",'" + campo.estado + "','" + campo.imagen +'\')">Editar</a><a class="important dropdown-item pl-2" href="javascript:void(0);" onclick="eliminarCategoria(' + campo.id_catg + ')">Eliminar</a><a href="javascript:void(0)" class="important dropdown-item pl-2" onclick="listarProductos(' + campo.id_catg + ')">Ver producto</a></li></div></div></div></div>'))
      ...
            }
        ...
        "columns":[
            {"data":null,"render": function ( data, type, row) {
                return '<a href="javascript:void(0)" class="link-2" onclick="listarPresentaciones('+data.id_prod+',\''+data.nombre+'\')">'+data.nombre+'</a>';
            }},
            {"data":null,"render": function ( data, type, row) {
                if(data.id_tipo == 1){
...
var listarPresentaciones = function(id_prod,nombre){
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(item){
        $('#head-p').html('<button class="btn btn-success btn-block btn-nuevo-presentacion" onclick="nuevaPresentacion('+id_prod+',\''+nombre+'\')"><i class="fas fa-plus-circle"></i> Nuevo </button>');
    ...
    $.ajax({
        ...
        success: function(item){
            $.each(item.data, function(i, campo) {
                ...
                $("#precio_compra").val(campo.precio_compra);
                ...
                if(campo.TipoProd.id_tipo == 1){
                    if(campo.receta == 1){
                        ...
                        $('#mensaje-ins').html('<div class="alert alert-info">'
                            +'Para modificar los ingredientes click <a class="alert-link-2" onclick="receta()">AQUI</a>'
                            +'</div>');
                    } else {
                        $('#receta_presentacion').prop('checked', false);
                        $('#hidden_receta_presentacion').val(0);
                        $('#mensaje-ins').css('display','none');
                        $('#mensaje-ins').html('<div class="alert alert-warning">'
                            +'Ingresar los ingredientes <a class="alert-link-2" onclick="receta()">AQUI</a> y luego click en Guardar'
                            +'</div>');
                    }
        ...
        success: function (data){
            ...
            $.each(data, function(i, item) {
                ...
                if(item.id_tipo_ins == 1){
                    var tipo = '<span class="text-white badge bg-warning">INSUMO</span>';
