    ...
    .DataTable({
        ...
        "columns":[
            ...
            {"data":null,"render": function ( data, type, row) {
                if(data.estado == 'a'){
                  return '<span class="text-white badge bg-success">ACTIVO</span>';
...
var listarMesas = function(id_salon,descripcion){
    ...
    $('#btn-nuevo').html('<button type="button" class="btn btn-circle btn-lg btn-orange waves-effect waves-dark" onclick="editarMesa('+mesaNueva+');"><i class="ti-plus"></i>Añadir Mesa</button>');
    ...
    .DataTable({
        ...
        "columns":[
            ...
            {"data":null,"render": function ( data, type, row) {
                if(data.estado == 'a'){
                  return '<a onclick="estadoMesa('+data.id_mesa+');"><span class="text-white badge bg-success">ACTIVO</span></a>';
                } else if (data.estado == 'i' || data.estado == 'p'){
                  return '<span class="label label-warning">OCUPADO</span>'
                }
                else if (data.estado == 'm'){
                  return '<a onclick="estadoMesa('+data.id_mesa+');"><span class="badge bg-danger text-white">INACTIVO</span></a>'
                } 
            }},
            {"data":null,
            class: "text-right",
            "render": function ( data, type, row ) {
            return '<div class="dropdown-action"><div class="dropdown todo-action-dropdown"><button class="btn btn-outline-primary dropdown-toggle text-decoration-none todo-action-dropdown" type="button" id="more-action-3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="icon-options-vertical"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="more-action-3" style="overflow:auto;"><a class="edit dropdown-item pl-2" href="javascript:void(0);" onclick="editarMesa(' + data.id_mesa + ",'" + data.nro_mesa + "','" + data.estado + '\');">Editar</a><a class="important dropdown-item pl-2" href="javascript:void(0);" onclick="eliminarMesa(' + data.id_mesa + ",'" + data.nro_mesa + "');\">Eliminar</a></div></div></div>";
            }}
        ]
    });
