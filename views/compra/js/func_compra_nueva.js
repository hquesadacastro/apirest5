$(function() {
    ...
    $('#c-compras-nuevo').addClass("active");
    ...
    .on('success.form.fv', function(e) {
        ...
        if($('#filtro_cuotas').val() == 0){
            ...
            return false;
                ...
                compra.detalle.adicional = $('#total_adicional').val();
...
$('#btn-agregar-insumo').click(function() {
    if($('#id_ins_buscar').val() == '' || $('#id_ins_buscar').val() == '0' || $('#cantidad_buscar').val() == '' || $('#precio_buscar').val() == ''){
        ...
        compra.registrar({
            ...
            precio_insumo: parseFloat($("#precio_buscar").val()).toFixed(5),
        });
        $("#id_ins_buscar").val('');
        $("#buscar_insumo").val('');
        $("#cantidad_buscar").val('');
...
var montoTotal = function(){
    var sub_total = $("#subtotal_global").val(),
        ...
        total_adicional = $("#total_adicional").val();
    if($("#tipo_compra").val() == 1){
        total = parseFloat(sub_total) - parseFloat(descuento_aumento) + parseFloat(total_adicional);
...
$('#total_adicional').on('keyup', function(){
    montoTotal();   
});
...
$('#modal-credito').on('hidden.bs.modal', function() {
    ...
    if($('#total_credito').val() == '' || $('#total_credito').val() == '0.00' || $('#cuotas_credito').val() == '0' || $('#cuotas_credito').val() == '' || $('input[name="fecha_cuota[]"]').val() == ''){
        ...
        $('#mensaje-credito').html('<div class="alert alert-danger m-b-0">'
            +'Adevertencia, Completar campos en blanco <a href="javascript:void(0)" class="alert-link" data-toggle="modal" data-target="#modal-credito"><span class="label label-success-2">AQUI</span></a>.</div>');
    }else{
        $('#filtro_cuotas').val('1');
        $('#mensaje-credito').html('<div class="alert alert-info m-b-0">'
            +'Click <a href="javascript:void(0)" class="alert-link" data-toggle="modal" data-target="#modal-credito"><span class="label label-success-2">AQUI</span></a> si necesita modificar las cuotas de la compra.'
            +'</div>');
    }
});
