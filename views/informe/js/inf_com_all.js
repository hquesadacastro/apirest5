...
var listar = function(){
	...
	.DataTable({
        buttons: [
            {
                extend: 'excel', title: 'rep_compras', text:'Excel', className: 'btn btn-circle btn-lg btn-success waves-effect waves-dark', text: '<i class="mdi mdi-file-excel display-6" style="line-height: 10px;"><span class="botones-excel">Exportar excel</span></i>', titleAttr: 'Descargar Excel',
                container: '#btn-excel'
            }
        ],
		"destroy": true,
        ...
                return '<div class="text-right bold">'+moneda+' '+formatNumber(data)+'</div>';
            }},
            {
                ...
                "render": function ( data, type, row) {
                    if(data.id_tipo_compra == 1){
                        return '<span class="text-white badge bg-primary">'+data.desc_tc+'</span>';
                    } else if(data.id_tipo_compra == 2){
                        return '<span class="text-white badge bg-warning">'+data.desc_tc+'</span>';
                    }
                }
            },
            {
                ...
                "render": function ( data, type, row ) {
                    if(data.estado == 'a'){
                        return '<div class="text-center"><span class="text-white badge bg-success">APROBADO</span></div>';
                    } else if(data.estado == 'i'){
                        return '<div class="text-center"><span class="text-white badge bg-danger">ANULADO</span></div>';
                    }
                }
            },
            {"data":null,
            class: "text-right",
            "render": function ( data, type, row ) {
                    return '<div class="dropdown-action">    <div class="dropdown todo-action-dropdown"><button class="btn btn-outline-primary dropdown-toggle text-decoration-none todo-action-dropdown" type="button" id="more-action-3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="icon-options-vertical"></i>        </button>        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="more-action-3" style="overflow:auto;">            <a class="dropdown-item" href="javascript:void(0);" onclick="detalle_cuota(' + data.id_compra + ');">Cuotas</a>            <a class="dropdown-item" href="javascript:void(0);" onclick="detalle(' + data.id_compra + ')">Detalle</a>        </div>    </div></div>';
            }}
		],
    ...
    $.ajax({
      ...
      success: function (data){
        $.each(data, function(i, item) {
            var importe = (parseFloat(item.Compra.total) - parseFloat(item.Compra.descuento)) + parseFloat(item.Compra.adicional);
            var cantidad = (item.cant * 1);
            var precio = (item.precio * 1);
            ...
            .append(
              $('<tr/>')
                ...
                .append($('<td/>').html(item.cant+' <span class="p-1 badge badge-warning">'+item.Producto.ins_med+'</span>'))
                .append($('<td/>').html(moneda+' '+precio.toFixed(5)))
...
var detalle_cuota = function(id_compra){
    ...
    $.ajax({
        ...
        success: function(data){
            $.each(data, function(i, item) {
                if (item.estado == 'p'){
                    label = 'p-10 badge badge-success';
                    nombre = 'EN PAGO';
                ...
                .append(
                  $('<tr/>')
                    ...
                    .append($('<td class="text-right"/>')
                        .html('<button class="btn btn-xs btn-info" onclick="detalle_sub_cuota('+item.id_credito+')"><i class="fas fa-eye"></i>Ver cuotas</button>'))
                    );
            });
        }
    });
