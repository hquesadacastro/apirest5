...
var defaultdata = function(){
    $.ajax({
        ...
        success: function (item) {
            ...
            var totalIng = (parseFloat(item.pago_efe) + parseFloat(item.Ingresos.total)).toFixed(2);
            $('.c-total-ingreso').html(moneda+' '+formatNumber(totalIng));
            console.log(totalIng)

            var totalEgr = (parseFloat(item.EgresosA.total) + parseFloat(item.EgresosB.total));
            ...
            //var montoEfectivo = ((parseFloat(totalIng) - parseFloat(item.pago_tar) - parseFloat(item.pago_yape) - parseFloat(item.pago_plin) - parseFloat(item.pago_lin) - parseFloat(item.pago_tra)) - parseFloat(totalEgr)).toFixed(2);

            var montoEfectivo = parseFloat(item.pago_efe).toFixed(2);
            $(".c-monto-efectivo").html(moneda+' '+formatNumber(montoEfectivo));


            
            var montoDiferencia = ((parseFloat(item.Apertura.monto_aper) + parseFloat(totalIng) - parseFloat(totalEgr)) - parseFloat(item.Apertura.monto_cierre)).toFixed(2);
            $(".c-monto-diferencia").html(moneda + " " + formatNumber( montoDiferencia * -1)),
            montoDiferencia > 0 ? ($(".c-monto-diferencia").addClass("text-danger"),
            $(".name-c-monto-diferencia").html("(Faltante)")) : montoDiferencia < 0 ? ($(".c-monto-diferencia").addClass("text-success"),
            $(".name-c-monto-diferencia").html("(Sobrante)")) : $(".name-c-monto-diferencia").html("");

            $(".c-monto-tarjeta").html(moneda+' '+formatNumber(item.pago_tar));

            $(".c-monto-yape").html(moneda+' '+formatNumber(item.pago_yape));

            $(".c-monto-plin").html(moneda+' '+formatNumber(item.pago_plin));

            $(".c-monto-lin").html(moneda+' '+formatNumber(item.pago_lin));

            $(".c-monto-tra").html(moneda+' '+formatNumber(item.pago_tra));
