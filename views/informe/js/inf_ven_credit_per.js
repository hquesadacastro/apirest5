...
var listar = function(){
    ...
    .DataTable({
        buttons: [
            {
                extend: 'excel', title: 'Cortesias', className: 'dropdown-item', text: '<i class="fas fa-file-excel"></i> Descargar en excel', titleAttr: 'Descargar Excel',
                container: '#excel', exportOptions: { columns: [0,1,2,3,4,5,6] }
            },
            {
                extend: 'pdf', title: 'Cortesias', className: 'dropdown-item', text: '<i class="fas fa-file-pdf"></i> Descargar en pdf', titleAttr: 'Descargar Pdf',
        ...
        "ajax":{
            "method": "POST",
            "url": $('#url').val()+"informe/venta_credit_per_list",
            "data": {
                ifecha: ifecha,
                ffecha: ffecha,
                desc_tipo
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            $('.credit-per-total').text(moneda+' '+formatNumber(total));
            $('.credit-per-operaciones').text(operaciones);
        }
    });
}