    ...
    .DataTable({
        ...
            }},
            {"data":null,"render": function ( data, type, row ) {
                if(data.estado == 'a'){
                    return '<div class="text-center"><a href="javascript::void(0)" onclick="estado('+data.id_cliente+',\''+data.estado+'\',\''+data.nombre+'\''+');"><span class="text-white badge bg-success">ACTIVO</span></a></div>';
                }else if(data.estado == 'i'){
                    return '<div class="text-center"><a href="javascript::void(0)" onclick="estado('+data.id_cliente+',\''+data.estado+'\',\''+data.nombre+'\''+');"><span class="text-white badge bg-danger">INACTIVO</span></a></div>';
                }
            }},
            {"data":null,
            class: "text-right",
                "render": function ( data, type, row ) {
                return '<div class="dropdown-action"><div class="dropdown todo-action-dropdown"><button class="btn btn-outline-primary dropdown-toggle text-decoration-none todo-action-dropdown" type="button" id="more-action-3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="icon-options-vertical"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="more-action-3" style=""><a class="edit dropdown-item" href="javascript:void(0);" onclick="editar(' + data.id_cliente + ');">Editar</a><a class="important dropdown-item" href="javascript:void(0);" onclick="anular(' + data.id_cliente + ",'" + data.nombre + "');\">Eliminar</a></div></div></div>"
            }}
        ],
