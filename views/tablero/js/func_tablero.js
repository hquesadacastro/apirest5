...
$.ajax({
    ...
    success: function(item){
        ...
        var totalVentas = (parseFloat(item['Ventas'].pago_efe) + parseFloat(item['Ventas'].pago_tar) + parseFloat(item['Ventas'].pago_bil) + parseFloat(item['Ventas'].pago_tra) + parseFloat(item['Ventas'].pago_lin) + parseFloat(item['Ventas'].comis_tar)).toFixed(2);
        ...
        var tarjeta = (parseFloat(item['Ventas'].pago_tar) + parseFloat(item['Ventas'].pago_bil) + parseFloat(item['Ventas'].pago_tra) + parseFloat(item['Ventas'].pago_lin)* 100 ) / parseFloat(totalVentas);
        ...
        $('.pago_tar').text(moneda+" "+formatNumber(parseFloat(item['Ventas'].pago_tar) + parseFloat(item['Ventas'].pago_bil) + parseFloat(item['Ventas'].pago_tra) + parseFloat(item['Ventas'].pago_lin) + parseFloat(item['Ventas'].comis_tar)));
        ...
        */
        ...
        if(item['Platos'].length > 0){
            ...
            $.each(item['Platos'], function(i, dato) {
                ...
                $('#lista_platos')
                    ...
                    .append(
                        $('<td/>')
                        .html('<h6>'+dato.pro_pre+'</h6><small class="text-muted">'+dato.pro_nom+'</small>')
                    )
                    .append(
                        $('<td/>')
            ...
            $.each(item['Productos'], function(i, datu) {
                ...
                $('#lista_productos')
                  .append(
                    ...
                    .append(
                        $('<td/>')
                        .html('<h6>'+datu.pro_pre+'</h6><small class="text-muted">'+datu.pro_nom+'</small>')
                    )
                    .append(
                        $('<td/>')
        ...
        if(item['StockMinimo']['Productos'].length > 0){
            var cont = 1;
            $.each(item['StockMinimo']['Productos'], function(i, datu) {
                $('#lista_productos_agotarse')
                  .append(
                    $('<tr/>')
                    .append(
                        $('<td style="width:50px;"/>')
                    ...
                    .append(
                        $('<td/>')
                        .html('<h6>'+datu.presentacion+'</h6><small class="text-muted">'+datu.nombre+'</small>')
                    )
                    .append(
                        $('<td class="text-center"/>')
                        .html(formatNumber(datu.stock_actual))
                    )
                    .append(
                        $('<td class="text-center"/>')
                        .html(datu.estado_actual)
                    )
                    .append(
                        $('<td class="text-center text-success"/>')
                        .html('<a href="'+datu.url+'compra/nueva-compra"><button class="btn btn-accesos-rapidos waves-effect waves-light border-0" type="button"><span class="btn-label0"><img src="'+datu.url+'public/images/carrito.png" width="20px" height="20px"/></span></button></a>')
                    )
                )
            });
        } else {
            $('#lista_productos_agotarse').html("<tr style='border-left: 2px solid #fff !important; background: #fff !important;'><td colspan='5'><div class='text-center'><h4 class='m-t-40' style='color: #d3d3d3;'><i class='mdi mdi-receipt display-3 m-t-40 m-b-10'></i><br>Realice una venta<br><small>No se encontraron datos <br>en el periodo de tiempo seleccionado</small></h4></div></td></tr>");
        }  
        
        if(item['StockMinimo']['Insumos'].length > 0){
            var cont = 1;
            $.each(item['StockMinimo']['Insumos'], function(i, datu) {
                $('#lista_insumos_agotarse')
                  .append(
                    $('<tr/>')
                    .append(
                        $('<td style="width:50px;"/>')
                    ...
                    .append(
                        $('<td/>')
                        .html('<h6>'+datu.nomb_ins+'</h6><small class="text-muted">'+datu.descripcion+'</small>')
                    )
                    .append(
                        $('<td class="text-center"/>')
                        .html(formatNumber(datu.stock_actual))
            ...
            $('#lista_insumos_agotarse').html("<tr style='border-left: 2px solid #fff !important; background: #fff !important;'><td colspan='5'><div class='text-center'><h4 class='m-t-40' style='color: #d3d3d3;'><i class='mdi mdi-receipt display-3 m-t-40 m-b-10'></i><br>Realice una venta<br><small>No se encontraron datos <br>en el periodo de tiempo seleccionado</small></h4></div></td></tr>");
        }          
    }
  });
}